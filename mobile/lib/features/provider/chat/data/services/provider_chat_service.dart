// lib/features/provider/chat/data/services/provider_chat_service.dart
...
class ProviderChatService {
  ...
  Future<ChatConversationModel> getConversation({
    ...
    final response = await http.get(
      _uri('/provider/conversations/$conversationId'),
      headers: _jsonHeaders(token),
    );
    ...
    if (data is! Map) {
      throw ProviderChatException('La conversación recibida no es válida.');
    }
  ...
  Future<List<ChatMessageModel>> getMessages({
    ...
    final response = await http.get(
      _uri('/provider/conversations/$conversationId/messages'),
      headers: _jsonHeaders(token),
    );
  ...
  Future<ChatMessageModel> sendMessage({
    ...
    if (cleanMessage.isEmpty && image == null) {
      throw ProviderChatException('Escribe un mensaje o selecciona una imagen.');
    }
    ...
    if (data is! Map) {
      throw ProviderChatException('El mensaje recibido no es válido.');
    }
  ...
  Future<ChatConversationModel> closeConversation({
    required String? token,
    required int conversationId,
  }) async {
    _ensureAuthenticated(token);
    ...
    final response = await http.post(
      _uri('/provider/conversations/$conversationId/close'),
      headers: _jsonHeaders(token),
    );
    ...
    if (response.statusCode != 200) {
      throw ProviderChatException(
        body['message']?.toString() ??
            'No se pudo cerrar esta conversación.',
      );
    }
  ...
  Future<void> markAsRead({
    ...
    final response = await http.post(
      _uri('/provider/conversations/$conversationId/read'),
      headers: _jsonHeaders(token),
    );
  ...
  Future<int> getUnreadCount({
    ...
    final response = await http.get(
      _uri('/provider/conversations/unread-count'),
      headers: _jsonHeaders(token),
    );
  ...
  void _ensureAuthenticated(String? token) {
    if (token == null || token.trim().isEmpty) {
      throw ProviderChatException('No hay token de autenticación.');
    }
  }
...
class ProviderChatException implements Exception {
  ...
  const ProviderChatException(this.message);
