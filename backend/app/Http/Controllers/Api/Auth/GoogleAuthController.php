...
class GoogleAuthController extends Controller
{
    ...
    public function __invoke(Request $request): JsonResponse
    {
        ...
        if (! $email) {
            return response()->json([
                'message' => 'No pudimos obtener el correo de tu cuenta de Google.',
            ], 422);
        }

        if (! $firebaseUid) {
            return response()->json([
                'message' => 'No pudimos obtener el identificador de tu cuenta de Google.',
            ], 422);
        }
        ...
        if ($existingUser && $existingUser->type !== 'customer') {
            return response()->json([
                'message' => 'Esta cuenta ya existe como afiliado. Por ahora Google Auth está habilitado solo para clientes.',
            ], 403);
        }
        ...
        $token = $user
            ->createToken('customer-google')
            ->plainTextToken;

        return response()->json([
            'message' => $requiresLegalOnboarding
                ? 'Cuenta de Google validada. Completa tu información legal para continuar.'
